...
from _serverApp import print_changes

from flask import json
# from flask_sqlalchemy import SQLAlchemy
from sqlalchemy.orm.attributes import QueryableAttribute
#from sqlalchemy_utils import UUIDType
#from _database_UUID_class import UUIDType,GUID
# import uuid
import base64
import uuid
import datetime
...
# engine = create_engine('sqlite:///C:\\Users\\User\\Documents\\my Projects\\Systems_Development\\Development_Environment\\ganimides.db')
...
class BaseModel(Base):
    ...
    def update_from_dict(self, **kwargs):
        ...
        heading = f"[{self.__tablename__}] [[changes]]:"
        if self.debug_is_on() or thisApp.application_configuration.database_models_debug: print_changes(heading, changes)
        return changes
    ########################################################################################
...
class Users(BaseModel):
    __tablename__ = 'users'
    id = Column(CHAR(36), default=get_uuid(), primary_key=True, unique=True, index=True)
...
class API(BaseModel):
    ...
    api_id = Column(CHAR(36), default=get_uuid(), unique=True, index=True)
    api_name = Column(VARCHAR(255) ,primary_key=True, nullable=False)
    status = Column(VARCHAR(50), default='Active')
    row_timestamp = Column(DateTime ,server_default=func.now())
    last_usage_timestamp = Column(DateTime ,server_default=func.now())
    def get_id(self):
        return (self.api_id)
    _debug = True
# # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # 
class APPLICATION(BaseModel):
    ...
    application_id = Column(CHAR(36), default=get_uuid(), unique=True, index=True)
    ...
    rowDT = Column(DateTime ,server_default=func.now())
    status = Column(VARCHAR(50))
    row_timestamp = Column(DateTime ,server_default=func.now())
# # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # 
class APPLICATION_API(BaseModel):
    __tablename__ = 'application_apis'
    application_api_id = Column(CHAR(36), default=get_uuid(), unique=True, index=True)
    application_id = Column(VARCHAR(255) ,ForeignKey(APPLICATION.application_id,name='fk_applications_application_id') ,primary_key=True, nullable=False)
    api_name = Column(VARCHAR(255) ,ForeignKey(API.api_name,name='fk_apis_api_name') ,primary_key=True, nullable=False)
    status = Column(VARCHAR(50) ,default='Active')
    #last_update_by = Column(VARCHAR(50))
    row_timestamp = Column(DateTime ,server_default=func.now())
    last_usage_timestamp = Column(DateTime ,server_default=func.now())
...
class CLIENT(BaseModel):
    ...
    client_id = Column(CHAR(36), default=get_uuid(), unique=True, index=True)
    email = Column(VARCHAR(255) ,primary_key=True, nullable=False)
    client_type = Column(VARCHAR(50) ,primary_key=True, nullable=False ,default='consumer')
    ...
    # shalimar = Column(VARCHAR(50))
    password = Column(VARCHAR(50))
    row_timestamp = Column(DateTime ,server_default=func.now())
    confirmed_timestamp = Column(DateTime)
    last_usage_timestamp = Column(DateTime ,server_default=func.now())
    #last_update_timestamp = Column(DateTime ,default=datetime.datetime.utcnow,onupdate=datetime.datetime.utcnow)
    bank_subscriptions = relationship("BANK_SUBSCRIPTION")
    bank_accounts = relationship("BANK_ACCOUNT")
# # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # 
class TOKEN(BaseModel):
    __tablename__ = 'tokens'
    token_id = Column(CHAR(36), default=get_uuid(),primary_key=True)
...
class MERCHANT(BaseModel):
    ...
    merchant_id = Column(CHAR(36), default=get_uuid(), unique=True, index=True)
...
class POINT_OF_SALE(BaseModel):
    ...
    pointofsale_id = Column(CHAR(36), default=get_uuid(), unique=True, index=True)
    ...
    address = Column(VARCHAR(255))
    shortAddress = Column(VARCHAR(255))
    geolocation_lat = Column(DECIMAL(12,6) ,default=0)
    geolocation_lon = Column(DECIMAL(12,6) ,default=0)
    bank_account_id = Column(VARCHAR(255))
    bank_subscription_id = Column(VARCHAR(255))
    bank_code = Column(VARCHAR(50))
    bank_subscriptionID = Column(VARCHAR(50))
    ...
    signed_tellerID_geolocation_lat = Column(DECIMAL(12,6) ,default=0)
    signed_tellerID_geolocation_lon = Column(DECIMAL(12,6) ,default=0)
    status = Column(VARCHAR(50))
    # last_update_by = Column(VARCHAR(50))
    row_timestamp = Column(DateTime ,server_default=func.now())
    # last_update_timestamp = Column(DateTime ,server_default=func.now())
...
class MERCHANT_EMPLOYEE(BaseModel):
    ...
    employee_id = Column(CHAR(36), default=get_uuid(), unique=True, index=True)
    merchant_id = Column(VARCHAR(50) ,ForeignKey(MERCHANT.merchant_id,name='fk_merchants_merchant_id', ondelete='CASCADE') ,primary_key=True, nullable=False)
    ...
    last_signon_geolocation_lat = Column(DECIMAL(12,6) ,default=0)
    last_signon_geolocation_lon = Column(DECIMAL(12,6) ,default=0)
    status = Column(VARCHAR(50))
    # last_update_by = Column(VARCHAR(50))
    row_timestamp = Column(DateTime ,server_default=func.now())
    # last_update_timestamp = Column(DateTime ,server_default=func.now())
...
class CONSUMER(BaseModel):
    ...
    consumer_id = Column(CHAR(36), default=get_uuid(), unique=True, index=True)
    email = Column(VARCHAR(255) ,primary_key=True, nullable=False)
    client_id = Column(VARCHAR(50) ,ForeignKey(CLIENT.client_id,name='fk_clients_client_id', ondelete='CASCADE'))
    mobile = Column(VARCHAR(255))
    # id = Column(Integer)
    # consumer_secretKey = Column(VARCHAR(255))
...
class BANK(BaseModel):
    __tablename__ = 'banks'
    bank_id = Column(CHAR(36), default=get_uuid(), unique=True, index=True)
    bank_BIC = Column(VARCHAR(50) ,primary_key=True, nullable=False)
    bank_code = Column(VARCHAR(50) ,unique=True, index=True)
...
class BANK_SUBSCRIPTION(BaseModel):
    ...
    bank_subscription_id = Column(CHAR(36), default=get_uuid(), unique=True, index=True)
...
class BANK_AUTHORIZATION(BaseModel):
    ...
    bank_authorization_id = Column(CHAR(36), default=get_uuid(), primary_key=True, unique=True, index=True)
...
class BANK_ACCOUNT(BaseModel):
    ...
    bank_account_id = Column(CHAR(36), default=get_uuid(), unique=True, index=True)
...
class DEVICE(BaseModel):
    ...
    status = Column(VARCHAR(50))
    row_timestamp = Column(DateTime ,server_default=func.now())
    last_usage_timestamp = Column(DateTime ,server_default=func.now(),onupdate=func.now())
    times_used = Column(Integer ,server_default=text('0'))
# # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # 
class DEVICE_USAGE(BaseModel):
    __tablename__ = 'device_usage'
    row_id = Column(CHAR(36), default=get_uuid(), unique=True, index=True)
    ...
    client_id = Column(VARCHAR(50) ,primary_key=True, nullable=False)
    geolocation_lat = Column(DECIMAL(12,6) ,primary_key=True, nullable=False ,default=0)
    geolocation_lon = Column(DECIMAL(12,6) ,primary_key=True, nullable=False ,default=0)
...
class CLIENT_DEVICE(BaseModel):
    ...
    client_device_id = Column(CHAR(36), default=get_uuid(), unique=True, index=True)
    device_uid = Column(VARCHAR(255) ,ForeignKey(DEVICE.device_uid, name='fk_devices_device_uid', ondelete='CASCADE') ,primary_key=True, nullable=False)
    client_id = Column(VARCHAR(50) ,ForeignKey(CLIENT.client_id, name='fk_clients_client_id', ondelete='CASCADE') ,primary_key=True, nullable=False)
    # application_id = Column(VARCHAR(50) ,ForeignKey(APPLICATION.application_id, name='fk_applications_application_id', ondelete='CASCADE') ,primary_key=True, nullable=False)
...
class INTERACTION(BaseModel):
    ...
    interaction_id = Column(CHAR(36), default=get_uuid(), primary_key=True)
    consumer_id = Column(VARCHAR(50) ,ForeignKey(CONSUMER.consumer_id, ondelete='CASCADE'))
...
class INTERACTION_MESSAGE(BaseModel):
    ...
    interaction_message_id = Column(CHAR(36), default=get_uuid(),primary_key=True)
    interaction_id = Column(VARCHAR(255) ,ForeignKey(INTERACTION.interaction_id,name='fk_interactions_interaction_id', ondelete='CASCADE'))
    originator_id = Column(VARCHAR(50))
    originator = Column(VARCHAR(50))
    message_type = Column(VARCHAR(50))
    message_record = Column(VARCHAR(1024))
    content_type = Column(VARCHAR(50))
    message_format = Column(VARCHAR(50))
    row_timestamp = Column(DateTime ,server_default=func.now())
# # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # 
# # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # 
# # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # 
...
class TEST(BaseModel):
    ...
    row_id = Column(CHAR(36), default=get_uuid(),primary_key=True)
    #device_uid = Column(VARCHAR(255) ,ForeignKey(DEVICE.device_uid, name='fk_devices_device_uid', ondelete='CASCADE') ,primary_key=True, nullable=False)
